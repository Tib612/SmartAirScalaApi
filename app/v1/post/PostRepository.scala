...
/**
  ...
  */
@Singleton
class PostRepositoryImpl @Inject()(dbapi: DBApi)(
  ...
  val parser =
    int("id") ~ str("title") ~ str("content") map {
      case id ~ title ~ content =>
        PostData(PostId(id.toString()), title, content)
    }
  ...
  override def get(
    ...
    Future {
      ...
      db.withConnection { implicit c =>
        val code: PostData = SQL(
          """
              select * from post where id = {id};
          """
        ).onParams(id.toString())
          .as(parser.single)
        Option(code)
      }
